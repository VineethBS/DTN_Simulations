...
class Relay:
    ...
    def meet_destination(self, dest):
        while True:
            next_meeting_time = numpy.random.exponential(1.0 / self.meeting_rate)
            yield self.env.timeout(next_meeting_time)
            # while len(self.queue) > 0: # this is just to check the case when all packets are delivered at once
            if len(self.queue) > 0:
                dest.deliver_packet()
                if RELAY_TRACE:
                ...
                packet_id_to_remove = self.queue[random_packet_index]
                # packet_id_to_remove = self.queue[0]
                # packet_id_to_remove = self.queue[-1]

                if PACKET_TRACE:
                    self.outputfile.write('Packet,RDPktCopy,%f,%u\n' % (self.env.now, packet_id_to_remove))
                self.queue.remove(packet_id_to_remove)
                # if PACKET_TRACE:
