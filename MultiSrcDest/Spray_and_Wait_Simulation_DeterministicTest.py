...
class Source:
    def __init__(self, env, arrival_rate, initial_queue_length, flow_id, number_copies):
        ...
        if SOURCE_TRACE:
            print('Source,%u,0,Initialization,%u,%u' % (self.flow_id, self.service_stage, self.queue_length))
    ...
    def serve_packet(self):
        if self.queue_length > 0:
            if self.service_stage < self.number_copies:
                ...
                if SOURCE_TRACE:
                    print('Source,%u,%f,Service,%u,%u' % (self.flow_id, self.env.now, self.service_stage, self.queue_length))
                return self.flow_id
    ...
    def run(self):
        while True:
            next_arrival_epoch = 1.0 / self.arrival_rate
            yield self.env.timeout(next_arrival_epoch)
            self.queue_length = self.queue_length + 1
            
            if SOURCE_TRACE:
                print('Source,%u,%f,Arrival,0,%u' % (self.flow_id, self.env.now, self.queue_length))
...
class Relay:
    ...
    def meet_source(self, src):
        while True:
            next_meeting_time = 1.0 / self.meeting_rate
            ...
            if len(self.queue) < self.max_buffersize:
                ...
                if not packet_id == -1:
                    ...
                    if RELAY_TRACE:
                        print('Relay,%u,%f,SourceMeeting,%u,%u' % (self.relay_id, self.env.now, src.flow_id, len(self.queue)))
    ...
    def meet_destination(self, dest):
        while True:
            ...
            if dest.flow_id in self.queue:
                ...
                dest.deliver_packet()
                if RELAY_TRACE:
                    print('Relay,%u,%f,DestinationMeeting,%u,%u' % (self.relay_id, self.env.now, dest.flow_id, len(self.queue)))
...
src = Source(env, 1, 0, 1, 2)
dest = Destination(1)

relay = Relay(env, 1, 2, 10, [src], [dest])
