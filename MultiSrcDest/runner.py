...
import sys
import MGKApprox
import MGK_2SD_HeuristicApprox as SD2
...
for scenario in range(number_scenarios):
    ...
    for s in range(scenario_params.numSources):
        results_outfile.write(",Source%uArrival,Source%uService,Source%uAvgQLength,Source%uAvgDelay,Source%uApproxDelay1,Source%uApproxDelay2" % (s,s,s,s,s,s))
                ...
                for number_relays in np.arange(scenario_params.numRelayMin, \
                    ...
                    for number_copies in np.arange(scenario_params.numCopiesMin, \
                        ...
                        for s in range(scenario_params.numSources):
                            ...
                            approxdelay = MGKApprox.MGKApprox(arrival_rate, meeting_rate, \
                                                              number_relays, number_copies, scenario_params.numSources)
                            SDapproxdelay = SD2.MGKApprox(arrival_rate, scenario_params.commonArrivalRate, meeting_rate, \
                                                          number_relays, number_copies, scenario_params.numSources)
                            results_outfile.write(",%f,%f,%f,%f,%f,%f" % \
                                                  (sim_arrivalrate, sim_servicerate, sim_avgqueuelength, sim_avgdelay, approxdelay, SDapproxdelay))
